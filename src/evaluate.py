# evaluar estimaciones utilizando algunas métricas comunes

import numpy as np

def evaluate_allpoint(obs, est, threshold=0.1):
    # obs/est: [número de puntos, muestra]
    nstn = obs.shape[0]
    metric_values = np.nan * np.zeros([nstn, 17])
    for i in range(nstn):
...
def evaluate(obs, est, threshold=0.1):
    # umbral: utilizado para la evaluación de probabilidad

    obs, est = data_preprocess(obs, est)

    metric_names = ['CC', 'ME', 'RB', 'MAE', 'ARB', 'RMSE', 'NSE', 'KGE_G2009', 'KGE_K2012', 'KGE_P2018', 'KGE_T2021', 'POD', 'FOH', 'FAR', 'CSI', 'HSS', 'PBIAS']
    metric_values = np.nan * np.zeros(17)
    if len(obs) > 1:
        ...
        metric_values[16] = cal_PBIAS(obs, est, preprocess=False)
...
def cal_PBIAS(obs, est, preprocess=False):
    # Percent Bias (PBIAS)
    rb = cal_RB(obs, est, preprocess)
    return rb * 100
...
def cal_NSE(obs, est, preprocess=True):
    # Eficiencia de Nash-Sutcliffe (Nash y Sutcliffe 1970 -
    # https://doi.org/10.1016/0022-1694(70)90255-6)
    # un índice común para observaciones hidrológicas
    if preprocess:
        obs, est = data_preprocess(obs, est)
    if len(obs) > 1:
...
def cal_KGE_G2009(obs, est, preprocess=True):
    # Eficiencia de Kling-Gupta original (Gupta et al. 2009 -
    # https://doi.org/10.1016/j.jhydrol.2009.08.003)
    # calcular el error en la sincronización y la dinámica r (coeficiente de correlación de Pearson)
    # que también se puede calcular usando np.corrcoef
    if preprocess:
        obs, est = data_preprocess(obs, est)
    if len(obs):
...
def cal_KGE_K2012(obs, est, preprocess=True):
    ...
    if len(obs) > 1:
        ...
        r = np.sum((est - est_mean) * (obs - obs_mean), axis=0, dtype=np.float64) / \
            np.sqrt(np.sum((est - est_mean) ** 2, axis=0, dtype=np.float64) *
                    np.sum((obs - obs_mean) ** 2, dtype=np.float64))
        # calcular el error en la sestad del flujo gamma (evitando la correlación cruzada con
        # el sesgo dividiendo por la media)
        ...
        # calcular la Eficiencia de Kling-Gupta modificada KGE'
        KGE = 1 - np.sqrt((r - 1) ** 2 + (gamma - 1) ** 2 + (beta - 1) ** 2)
        KGEgroup = {'KGE': KGE, 'r': r, 'gamma': gamma, 'beta': beta}
    else:
        KGEgroup = {'KGE': np.nan, 'r': np.nan,
                    'gamma': np.nan, 'beta': np.nan}
    return KGEgroup  # o salida np.vstack((KGE, r, gamma, beta))


def cal_KGE_P2018(obs, est, preprocess=True):
    # Eficiencia de Kling-Gupta no paramétrica (Pool et al. 2018 -
    # https://doi.org/10.1080/02626667.2018.1552002)
    # calcular el error en la sincronización y la dinámica r (coeficiente de correlación de Spearman)
    if preprocess:
        obs, est = data_preprocess(obs, est)
    if len(obs) > 1:
        ...
        # calcular el error en la sincronización y la dinámica alfa (curva de duración del flujo)
        ...
        beta = np.mean(est, axis=0) / \
            np.mean(obs, axis=0, dtype=np.float64)
        # calcular la Eficiencia de Kling-Gupta no paramétrica KGEnp
        KGE = 1 - np.sqrt((r - 1) ** 2 + (alpha - 1) ** 2 + (beta - 1) ** 2)
        KGEgroup = {'KGE': KGE, 'r': r, 'alpha': alpha, 'beta': beta}
    else:
        KGEgroup = {'KGE': np.nan, 'r': np.nan,
...
def cal_KGE_T2021(obs, est, preprocess=True):
    # KGE" propuesto por Tang et al 2021
...
def cal_contingency(obs, est, Tre=0.1):
    # Tre: umbral de lluvia/no lluvia
    # POD(Probabilidad de Detección),FOH(frecuencia de aciertos)
    # FAR(Tasa de Falsas Alarmas), CSI(Índice de Éxito Crítico)
    # HSS(puntuación de habilidad de Heidke),Ebert et al. [2007]
    if np.isnan(Tre):
        POD = np.nan
        FAR = np.nan
        FOH = np.nan
